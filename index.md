...
O aplicativo solicita a permissão `RECORD_AUDIO`  exclusivamente para permitir que você interaja com o "Tutor Financeiro" através de comandos de voz.
* **Processamento:** O áudio capturado é convertido em texto ou enviado para processamento pela API do **Google Gemini (Firebase AI)**  para interpretar sua dúvida financeira.
...
Respeitamos a sua autonomia sobre os dados, que são armazenados primariamente no seu dispositivo ("Local First") utilizando o banco de dados **AppDatabase (Room)**.
